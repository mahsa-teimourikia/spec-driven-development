...
The repository boundary allows feature, implementation, and test changes but treats policy and exception sources as read-only or prohibited. A coding agent may decide only fields explicitly delegated with `agent_authority = decide`; it may propose other delegated choices and must not modify fixed controls.
