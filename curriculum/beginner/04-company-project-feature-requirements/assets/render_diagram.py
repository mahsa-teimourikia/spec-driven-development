"""Validate diagram-spec.json and deterministically render the Course 04 SVG."""
...
def render(spec: dict) -> str:
    ...
    parts = [
        ...
        <marker id="arrow-authority" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 Z" fill="#2F6BFF"/></marker>
        <marker id="arrow-relationship" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 Z" fill="#087F7B"/></marker>
        <marker id="arrow-control" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 Z" fill="#238636"/></marker>
        <marker id="arrow-stop" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 Z" fill="#D97706"/></marker>
        ...
        f'<text x="50" y="58" class="title">{escape(spec["title"])}</text>',
        '<text x="50" y="90" class="subtitle">Authority stays with its owner; local layers specialize, implement, evidence, and stop on drift.</text>',
    ]
    for group in spec["groups"]:
        bounds = group["bounds"]
        parts.append(f'<g filter="url(#shadow)"><rect x="{bounds["x"]}" y="{bounds["y"]}" width="{bounds["width"]}" height="{bounds["height"]}" rx="18" fill="{group["fill"]}" stroke="{group["stroke"]}"/></g><text x="{bounds["x"] + 24}" y="{bounds["y"] + 38}" class="group">{escape(group["label"])}</text>')
    edge_styles = {"authority": ("#2F6BFF", "authority"), "relationship": ("#087F7B", "relationship"), "control": ("#238636", "control"), "stop": ("#D97706", "stop")}
    for edge in spec["edges"]:
        stroke, marker = edge_styles[edge["kind"]]
        points = " ".join(f"{x},{y}" for x, y in edge["route"])
        parts.append(f'<polyline points="{points}" fill="none" stroke="{stroke}" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" marker-end="url(#arrow-{marker})"/>')
        if edge.get("label"):
    ...
    for node in spec["nodes"]:
        ...
        parts.append(f'<g><rect x="{x}" y="{y}" width="{bounds["width"]}" height="{bounds["height"]}" rx="14" fill="{palette["fill"]}" stroke="{palette["stroke"]}" stroke-width="2"/><text x="{x + 20}" y="{y + 42}" class="label">{escape(node["label"][0])}</text><text x="{x + 20}" y="{y + 73}" class="note">{escape(node["subtitle"])}</text></g>')
    parts.append("</svg>")
    return "\n".join(parts)
